...
if _jac_typ.TYPE_CHECKING:
    ...
    randint, = __jac_import__(target='random', base_path=__file__, lng='py', absorb=False, mdl_alias=None, items={'randint': None})
...
class Router(_Jac.Node):
    ...
    def classify(self, message: str) -> ChatType:
        return _Jac.with_llm(file_loc=__file__, model=llm, model_params={'method': 'Reason', 'temperature': 0.0}, scope='serverpt3(Module).Router(node).classify(Ability)', incl_info=[], excl_info=[], inputs=[('query from the user to be routed.', str, 'message', message)], outputs=('', 'ChatType'), action='route the query to the appropriate task type', _globals=globals(), _locals=locals())
...
class infer(_Jac.Walker):
    ...
    def init_router(self, _jac_here_: _Jac.RootType) -> None:
        if _Jac.visit_node(self, (lambda x: [i for i in x if isinstance(i, Router)])(_Jac.edge_ref(_jac_here_, target_obj=None, dir=_Jac.EdgeDir.OUT, filter_func=None, edges_only=False))):
            ...
            _Jac.connect(left=router_node, right=MotivationChat(), edge_spec=_Jac.build_edge(is_undirected=False, conn_type=None, conn_assign=None))
...
@_Jac.make_node(on_entry=[_Jac.DSFunc('respond', infer)], on_exit=[])
@__jac_dataclass__(eq=False)
class MotivationChat(Chat, _Jac.Node):
    chat_type: ChatType = _Jac.has_instance_default(gen_func=lambda: ChatType.MOTIVATION)

    def respond(self, _jac_here_: infer) -> None:
        affirmations = ['You are capable of amazing things!', 'Believe in yourself and all that you are.', 'Keep pushing forward, success is just around the corner.', 'Youâ\x80\x99ve got this!', 'Every day is a fresh start, take advantage of it!', 'You are stronger than you think.', 'Keep believing, the best is yet to come!']
        random_index = randint(0, len(affirmations) - 1)
        _jac_here_.response = affirmations[random_index]
        _Jac.report({'response': _jac_here_.response})
